...
module.exports = {
  async play(song, message) {
    ...
    if (!song) {
      ...
      return queue.textChannel.send({embed: {"description": `**🚫 Dinledğiniz Şarkı Bitti.**`, "color": "RANDOM"}});
    }
    ...
    try {
      var playingMessage = await queue.textChannel.send({embed: {"description": `**Just Wørmzz | 🎧 Müzik Başladı 🎧 \nBaşlık\n [${song.title}](${song.url}) \n Sarkıyı Açan \n ${message.author}\nSes Seviyesi \n${serverQueue.volume}%**`, "color": "BLUE"}});
